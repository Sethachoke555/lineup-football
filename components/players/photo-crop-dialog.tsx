...
import type { Player, PlayerPhotoSettings, Project } from '@/types/project';
...
export function PhotoCropDialog({ image, settings, player, project, onApply, onCancel }: {
  ...
  onApply: (settings: PlayerPhotoSettings) => void; onCancel: () => void;
}) {
  ...
  useEffect(() => {
    ...
    const card = preview.current!; card.width = 600; card.height = 660;
    const cardCtx = card.getContext('2d')!; cardCtx.translate(300, 320);
    drawPlayerCard(cardCtx, { ...player, photoSettings: crop }, project, 500, 525, image);
  }, [crop, image, player, project]);
  useEffect(() => {
